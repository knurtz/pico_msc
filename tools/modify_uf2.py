...
Syntax: python modify_uf2.py [FILENAME] [LIMIT_ADDRESS]        # address can either be DEC or 0xHEX
"""
...
# Wait until UF2 has been written completely
time.sleep(.5)
...
print("Source file created %.2f seconds ago." % (time.time() - os.path.getmtime(filename)))
print("Any section from address 0x%x and above will not be written to flash." % address_limit)
...
last_printed_block = 0          # remember the last block number that has been printed to the console
wanted_blocks = 0               # count the amount of blocks until address limit is reached
...
for i in range(0, total_blocks):
    ...
    # usually each block contains 256 conescutive bytes of the payload data to be flashed
    # do something if there is a jump of more than 256 bytes between blocks
    # that means we encountered a non consecutive part of the UF2, which we want to show in the console output
